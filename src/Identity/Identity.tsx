...
import { ValidateAddress, trimAddress, timeAgo, formatBlockTime } from '../utils/streamTools/WalletAddress'; // global key handling
import { STREAM_RPC_ENDPOINT, STREAM_PROFILE, STREAM_PREVIEW } from '../utils/streamTools/constants';
...
export function IdentityView(props: any){
    ...
    const fetchSolanaTokens = async () => {
        ...
        */
        ...
        const resp = await fetch(STREAM_RPC_ENDPOINT, {
            method: "POST",
                                                        ...
                                                        {solanaHoldings.length > 0 ? solanaHoldings.map((item: any) => (
                                                            <ListItem>
                                                                ...
                                                                <>
                                                                    <Tooltip title="NFT">
                                                                        <ListItemButton 
                                                                            component={Link} to={`${STREAM_PREVIEW}${item.account.data.parsed.info.mint}`}
                                                                            sx={{borderRadius:'24px'}}
                                                                        >
                                                                            <ListItemAvatar>
                                                                                <Avatar
