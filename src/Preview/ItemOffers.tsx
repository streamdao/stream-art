...
import { RegexTextField } from '../utils/streamTools/RegexTextField';
import { MakeLinkableAddress, ValidateCurve, trimAddress, timeAgo } from '../utils/streamTools/WalletAddress'; // global key handling
...
import { depositInTreasury } from '../utils/auctionHouse/depositInTreasury';
            ...
            <BootstrapDialog 
                ...
                PaperProps={{
                    ...
                    style: {
                        ...
                        background: 'linear-gradient(to right, #0F2027, #000000)',
                        boxShadow: '3',
...
export default function ItemOffers(props: any) {
    ...
    const GetSignatureOffers = async (spkey: string, until: any, slimit: Number)  => { // made this more generic of a function
        ...
        const response = await fetch(STREAM_RPC_ENDPOINT, {
          method: "POST",
    ...
    const getOffers = async () => {
        const anchorProgram = await loadAuctionHouseProgram(pubkey, ENV_AH, STREAM_RPC_ENDPOINT);
        const auctionHouseKey = new web3.PublicKey(AUCTION_HOUSE_ADDRESS);
        const auctionHouseObj = await anchorProgram.account.auctionHouse.fetch(auctionHouseKey,);
        ...
        try {
            ...
            //if (amount === 0){
                ...
                if (escrowAmount > 0){
                    ...
                    const transactionInstr = await depositInTreasury(depositAmount, buyerPublicKey);
                    const instructionsArray = [transactionInstr.instructions].flat();        
                    const transaction = new Transaction()
                    .add(
                        ...instructionsArray
                    ...
                    enqueueSnackbar(`${t('Preparing to Deposit amount back in Treasury')}: ${depositAmount} SOL ${t('to')}: ${buyerPublicKey.toBase58()}`,{ variant: 'info' });
                    const signedTransaction = await sendTransaction(transaction, connection);
                    ...
                    enqueueSnackbar(`${t('Deposit back to Treasury completed')}`,{ variant: 'success', action:snackaction });
                }
                const eskey = enqueueSnackbar(`${t('Metadata will be refreshed in a few seconds')}`, {
                        anchorOrigin: {
                            vertical: 'top',
                                ...
                                {publicKey && publicKey.toString() === mintOwner ? (
                                    ...
                                    >
                                        ...
                                        {t('Selling now')}: 
                                            ...
                                            <Typography component="div" variant="caption" id="stream-art-last-sale"></Typography>
                                        </Typography>
                                        {( (salePrice > 0) ?
                                                                    ...
                                                                    {( (salePrice > 0) ?
                                                                        <>
                                                                            ...
                                                                            <Button 
                                                                                ...
                                                                                sx={{
                                                                                    background: 'linear-gradient(268.11deg, #11ACD4 3.12%, #345C7B 96.88%)',
                                                                                    borderRadius: '10px',
                                                                                    color: '#fff',
                                                                                    mr:1,
                                                                                }}
                                                        ...
                                                        <TableRow>
                                                            <TableCell><Typography variant="body2">
                                                                <Tooltip title={t('View Profile')}>
                                                                    <Button size="small" variant="text" component={Link} to={`${STREAM_PROFILE}${item.buyeraddress}`} target="_blank" sx={{ml:1,color:'white',borderRadius:'24px'}}>
