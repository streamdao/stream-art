...
import { UPDATE_AUTHORITIES } from '../utils/streamTools/mintverification';
import { 
    ...
    STREAM_PROFILE,
    FEATURED_DAO_ARRAY,
    HQ9xdik6TD9wJHFDxQHoSJ4U9HWGP3xfVXLNDG3m2h7y,
    TOKEN_REPORT_AMOUNT,
...
import { MakeLinkableAddress, ValidateAddress, ValidateCurve, trimAddress, timeAgo } from '../utils/streamTools/WalletAddress'; // global key handling

import "../App.less";

import { useConnection, useWallet } from '@solana/wallet-adapter-react';
import { decodeMetadata } from '../utils/auctionHouse/helpers/schema';
import GrapeIcon from "../components/static/StreamIcon";
    ...
    if (verifiedState){
        ...
        return (
            <Tooltip title={`${props.symbol}: ${t('Update Authority/Creator Verified on Metaplex')}`} placement="top">
                <Button 
                    href={`${STREAM_PREVIEW}${verifiedPK}`}
...
function GalleryItemMeta(props: any) {
        ...
        if (publicKey){ 
            ...
            try{
                ...
                portfolio_rsp.map((token:any) => {
                    ...
                    if (mint === '5P3giWpPBrVKL8QP8roKM7NsLdi3ie1Nc2b5r9mGtvwb'){ // check if wallet has sol
                        if (governance_rsp?.account?.governingTokenDepositAmount){
    ...
    try{
        return (
            <Grid>
                <Helmet>
                    <title>{`${collectionitem.name} | ${t('StreamDAO Social. Stateless. NFT Marketplace.')}`}</title>
                    <meta property="og:title" content={`${collectionitem.name} @StreamDao`} />
                    ...
                    <meta name="twitter:site" content={`${collectionitem.name} @StreamDao`} />
                                    ...
                                    <Box display="flex" justifyContent="flex-end">
                                        <ButtonGroup variant="text">
                                            ...
                                            <SocialFlags mint={mint} />
                                                ...
                                                <Grid item>        
                                                    <ShareSocialURL fontSize={'24px'} url={window.location.href} title={'StreamDEX | '+trimAddress(mint,4)} />
                                                </Grid>
                                                                                    ...
                                                                                    {collectionitem.properties.creators.length > 0 && collectionitem.properties.creators.map((item: any) => (
                                                                                        <TableRow>
                                                                                            ...
                                                                                            <Button
                                                                                                ...
                                                                                                component={Link} 
                                                                                                to={`${STREAM_PROFILE}${item.address}`}
                                                                                            >
                                                            ...
                                                            <Grid container direction="row">
                                                                ...
                                                                <>
                                                                    ...
                                                                    <Tooltip title={t('Visit Profile')}>
                                                                        <Button
                                                                            component={Link} 
                                                                            to={`${STREAM_PROFILE}${tokenOwners?.data.parsed.info.owner}`}
                                                                            sx={{borderRadius:'24px', color:'white'}}
                                                                        >
                                ...
                                >
                                    ...
                                    >
                                        <Typography
                                            ...
                                            sx={{mt:2,mb:3}}
                                        >
                                            <img src="/stream_white_logo.svg" width="300px" className="header-logo" alt="StreamDAO" />
                                            .art
                                        </Typography>
                                    </Grid>
                                    <Grid 
