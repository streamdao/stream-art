...
import StreamIcon from '../components/static/StreamIcon';
...
import { STREAM_RPC_ENDPOINT, STREAM_RPC_REFRESH, STREAM_PREVIEW, STREAM_PROFILE, FEATURED_DAO_ARRAY } from '../utils/streamTools/constants';
...
export default function OffersView(props:any){
    ...
    const connection = new Connection(STREAM_RPC_ENDPOINT);
            ...
            if (allmints.length <= 1){
                if (mint){ // with mint allow calling cancel withdraw combo
                    ...
                    try {
                        ...
                        enqueueSnackbar(`${t('Withdrawal from Treasury completed')} `,{ variant: 'success', action:snackaction });
                ...
                //enqueueSnackbar(`To withdraw from Treasury you must first cancel all pending Offers.`,{ variant: 'warning' });
                    ...
                    for (var item of allmints){  
                        ...
                        try{
                            if (cnt === allmints.length){
                                ...
                                enqueueSnackbar(`${t('Treasury Withdrawal complete')} `,{ variant: 'success', action:snackaction });                     
                            }
                        } catch(e){
                            closeSnackbar();
                            enqueueSnackbar(`${t('Error')}: ${(e)}`,{ variant: 'error' });
                        ...
                        >
                            ...
                            <BootstrapDialog 
                                ...
                                <DialogContent>
                                    ...
                                    <Alert 
                                        ...
                                        >
                                            {t('You currently have')} <strong>{myoffers}</strong> {t('standing offer')}{(myoffers > 1 && <>s</>)}, {t('it is recommended that you cancel all standing offers and then attempt to withdraw. If you are unable to cancel then click Withdraw to force cancel from the Stream Auction House')}
                                            <br/><br/>
                            ...
                            >
                                <Typography variant="caption">
                                    <Button
                                            title={t('Withdraw from the Stream Auction House')}
                                            size="small"
                                            variant="text"
                                        ...
                                        <TableRow sx={{p:1}} key={key}>
                                            <TableCell>
                                                <Tooltip title={t('Visit Profile')}>
                                                    <Button
                                                        component={Link} to={`${STREAM_PROFILE}${item.buyeraddress}`}
                                            ...
                                            <TableCell align="right">
                                                <Tooltip title={t('View NFT')}>
                                                    <Button
                                                        component={Link} to={`${STREAM_PREVIEW}${item.mint}`}
                                                        sx={{borderRadius:'24px'}}
                                                    >
