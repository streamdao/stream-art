import { PublicKey, SystemProgram, TransactionInstruction, Transaction, Keypair, Connection} from '../streamTools/node_modules/@solana/web3.js.js'
...
//import { useAnchorWallet } from "@solana/wallet-adapter-react";
  ...
  export async function createProposal(offerAmount: number, mint: string, walletPublicKey: string, mintOwner: any, weightedScore: any, daoPublicKey: string, connection: Connection, transactionInstr: InstructionsAndSignersSet, sendTransaction: any, wallet: WalletSigner, state: number): Promise<PublicKey> {
    ...
    if (state===1) {    
      name = 'Offer for NFT: '+mint+' for '+offerAmount+'sol on STREAM.ART';
    } else if (state===2){
      name = 'Sale of NFT: '+mint+' for '+offerAmount+'sol on STREAM.ART';
    }  else {
      name = 'Unknown proposal for NFT: '+mint+' for '+offerAmount+'sol on STREAM.ART';
    }
    ...
    //console.log('signatoryRecordAddress:', signatoryRecordAddress.toBase58());
    
    const insertInstructions: TransactionInstruction[] = [];
    //we don't have any prerequisiteInstructions to execute so we will leave this null
    const prerequisiteInstructions: TransactionInstruction[] = [];

    //loop InsertTransactions based on number of instructions in transactionInstr
