...
import { STREAM_RPC_ENDPOINT, VERIFIED_DAO_ARRAY, } from '../streamTools/constants';
...
export async function voteOffer(offerAmount: number, mint: string, mintOwner: string, buyerAddress: string, walletPublicKey: string): Promise<InstructionsAndSignersSet> {
  ...
  //attempt to add as a signer solTreasury as first signer = fee payer by default
  ...
  instruction.keys
    ...
    .map(k => (k.isSigner = true, k.isWritable = true));
  //attempt to add auction house as keypair and assign as fee payer and see what happens
/* instruction.keys
